...
import { PUBLISHER_URI } from "../constants";
 ...
 */
export async function writeTtlFile(graph, path, logicalFileName, derivedFrom) {
  const phyId = uuid();
  const phyFilename = basename(path);
  const physicalFile = path.replace("/share/", "share://"); // FIXME, what if we don't map to /share?
  const loId = uuid();
      ...
      INSERT DATA {
        GRAPH ${sparqlEscapeUri(graph)} {
          ${sparqlEscapeUri(physicalFile)} a nfo:FileDataObject;
                                  nie:dataSource ${sparqlEscapeUri(
      logicalFile,
    )} ;
                                  prov:wasDerivedFrom ${sparqlEscapeUri(
      derivedFrom,
    )};
