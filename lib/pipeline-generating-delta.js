...
export async function run(deltaEntry) {
  ...
  try {
    ...
    const makeFileFromTriplestore =
      (fromDeltaToJson) => async (tripleStore, operation, derivedFrom) => {
        ...
        // DISABLED BECAUSE NOT SURE IF THAT DOES IMPROVE ANYTHING
        ...
        //   }, []);
        //
        // const tripleChunks = chunk(
        //   (await storeToString(tripleStore))
        //     .split("\n")
        //     .filter((n) => n?.length && n.trim().endsWith(".")),
        //   10,
        // );
        //
        // for (const triples of tripleChunks) {
        //   await update(
        //     `
        //    ${operation} {
    ...
    await getTriplesInFileAndApplyByBatch(
      ...
      async (tripleStore, derivedFrom) => {
        ...
        })(tripleStore, "DELETE WHERE", derivedFrom);
      },
    );
    ...
    await getTriplesInFileAndApplyByBatch(
      ...
      async (tripleStore, derivedFrom) => {
        ...
        })(tripleStore, "INSERT DATA", derivedFrom);
      },
    );
